{% load rest_framework %}
<div class="sidebar">
    <h3 class="brand"><a href="home">{{ document.title }}</a></h3>
    ...
    <div class="menu-list">
        ...
        <ul class="menu-list menu-list-bottom">
            ...
            <ul class="sub-menu collapse out" id="auth-control">
                <li {% if not user.is_authenticated %}class="active"{% endif %}><a data-auth="none" href="home">none</a></li>
                <li><a data-auth="token" data-toggle="modal" data-target="#auth_token_modal" href="home">token</a></li>
                <li><a data-auth="basic" data-toggle="modal" data-target="#auth_basic_modal" href="home">basic</a></li>
                <li {% if user.is_authenticated %}class="active"{% endif %}><a data-auth="session" data-toggle="modal" data-target="#auth_session_modal" href="home">session</a></li>
            </ul>
            ...
            {% for lang in langs %}
                <li{% if loop.first %} class="active"{% endif %}><a href="home" data-language="{{ lang }}">{{ lang }}</a></li>
	    {% endfor %}
            </ul>
        </ul>
